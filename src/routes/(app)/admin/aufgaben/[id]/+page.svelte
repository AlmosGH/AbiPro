...
<main>
	...
	{#if data.task.status === 'draft'}
		...
		<form method="POST" action="?/save" use:enhance={() => async ({ result, update }) => { if (result.type === 'success') dirty = false; await update({ reset: false }); }} oninput={() => dirty = true}>
			...
			<section><h2>Grunddaten</h2><p><label>Titel <input name="title" bind:value={title} required /></label></p><p><label>Anweisung <textarea name="instructions" bind:value={instructions}></textarea></label></p><p>Sammlung: {origin === 'official' ? 'Offizielle Prüfungen' : 'Újkor.hu'}</p><input type="hidden" name="origin" value={origin} /><p><label>Geschichte <select name="historyScope" bind:value={historyScope}><option value="global">Weltgeschichte</option><option value="hungarian">Ungarische Geschichte</option></select></label></p>{#if origin === 'official'}<p><label>Lehrplan <select name="curriculumId" bind:value={curriculumId}>{#each data.curricula as item (item.id)}<option value={item.id}>{item.name}</option>{/each}</select></label></p>{/if}<p><label>Epoche <select name="periodId" bind:value={periodId}>{#each data.periods as item (item.id)}<option value={item.id}>{item.name}</option>{/each}</select></label></p>{#if origin === 'official'}<p><label>Prüfungstermin <select name="examSessionId" bind:value={examSessionId}>{#each data.sessions as item (item.id)}<option value={item.id}>{item.year} – {item.session === 'spring' ? 'Frühjahr' : 'Herbst'}</option>{/each}</select></label></p><p><label>Offizielle Position im Kurzantwort-Teil <select name="examPosition" bind:value={examPosition}><option value={null}>Nicht für Prüfungen</option>{#each Array.from({ length: 12 }, (_, index) => index + 1) as position (position)}<option value={position}>{position}</option>{/each}</select></label></p>{/if}<p><label>Maximalpunktzahl <input name="maxPoints" bind:value={maxPoints} type="number" min="0.25" step="0.25" required /></label></p><fieldset><legend>Themen</legend>{#each data.topics as item (item.id)}<label><input type="checkbox" name="topicIds" value={item.id} bind:group={topicIds} /> {item.name}</label>{/each}</fieldset></section>
