...
<main>
	...
	{#if data.tasks.length}
		<ul>{#each data.tasks as task (task.slug)}<li><h2><a href={`/aufgaben/${task.slug}`}>{task.title}</a></h2><p><strong>{task.practiced ? 'Geübt' : 'Noch nicht geübt'}</strong></p><p>{task.year} · {task.session === 'spring' ? 'Frühjahr' : 'Herbst'} · {task.period} · {task.curriculum} · {task.maxPoints} Punkte</p><p>Themen: {task.topics.join(', ')}</p><p><a href={`/uben?task=${task.slug}`}>Diese Aufgabe üben</a></p></li>{/each}</ul>
