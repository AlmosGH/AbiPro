...
<main>
	...
	<p>{data.task.origin === 'ujkor' ? 'Újkor.hu-Sammlung' : `${data.task.year} · ${data.task.session === 'spring' ? 'Frühjahr' : 'Herbst'} · ${data.task.curriculum}`} · {data.task.period} · {data.task.historyScope === 'hungarian' ? 'Ungarische Geschichte' : 'Weltgeschichte'} · {data.task.maxPoints} Punkte</p>
