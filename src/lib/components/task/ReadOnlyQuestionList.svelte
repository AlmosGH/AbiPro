...
<ol>
	{#each questions as question (question.id)}
		<li>
			<p>{question.prompt} ({question.maxPoints} {language.t('P.')})</p>
			...
			{:else if question.config.kind === 'ordering'}
				<p>{language.t('In die richtige Reihenfolge bringen:')}</p>
				<ul>{#each question.config.items as item (item.id)}<li>{item.label}</li>{/each}</ul>
			{:else}
				<p>{language.t(question.config.multiline ? 'Mehrzeilige Freitextantwort' : 'Kurze Freitextantwort')}</p>
			{/if}
		</li>
	{/each}
</ol>
