...
const task = z.object({ position: z.number().int().min(1).max(12), title: z.string().min(1).max(200), instructions: z.string().max(5000), maxPoints: z.number().positive(), officialMaxPoints: z.number().positive().optional(), period, topics: z.array(topic).min(1), sources: z.array(source).min(1), questions: z.array(question).min(1) });
...
async function publish() {
	...
	try { await sql.begin(async (tx) => {
		...
		for (const entry of map.tasks) {
			...
			const [periodRow] = await tx<{ id: number }[]>`insert into app_private.historical_periods (slug, name, position) values (${data.period.slug}, ${data.period.name}, ${data.period.position}) on conflict (slug) do update set name = excluded.name, position = excluded.position returning id`;
			const topicIds: number[] = []; for (const value of data.topics) { const [row] = await tx<{ id: number }[]>`insert into app_private.topics (slug, name, period_id) values (${value.slug}, ${value.name}, ${periodRow.id}) on conflict (slug) do update set name = excluded.name, period_id = excluded.period_id returning id`; topicIds.push(row.id); }
			...
			const [version] = await tx<{ id: number }[]>`insert into app_private.task_versions (task_id, version, status, title, instructions, curriculum_id, period_id, exam_session_id, max_points, exam_position) values (${created.id}, ${created.next_version}, 'draft', ${data.title}, ${data.instructions}, ${curriculum.id}, ${periodRow.id}, ${sessionRow.id}, ${data.maxPoints}, ${data.position}) returning id`;
			for (const id of topicIds) await tx`insert into app_private.task_version_topics (task_version_id, topic_id) values (${version.id}, ${id})`;
